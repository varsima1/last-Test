...
//     name: Joi.object()
//       .keys({
//         first: Joi.string().min(2).max(256),// deleted required()
//         middle: Joi.string().min(2).max(256).allow(""),
//         last: Joi.string().min(2).max(256), // deleted required()
//       }),
...
const userUpdateValidation = (user) => {
  const schema = Joi.object({
    name: Joi.object().keys({
      first: Joi.string().min(2).max(256).allow(""),
      middle: Joi.string().min(2).max(256).allow(""),
      last: Joi.string().min(2).max(256).allow(""),
    }),
